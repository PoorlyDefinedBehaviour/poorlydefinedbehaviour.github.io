...
It is not that useful to crash a replica that's already in the crashed state or to crash more than `F` replicas if we are intereted in testing that a cluster with `2F + 1` replicas can make progress as long as the majority of replicas are up. In general, actions that generate failures should be generated according to the failure model.
